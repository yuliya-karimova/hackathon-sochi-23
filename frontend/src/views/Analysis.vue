<template>
  <div>
    <Header />
    <div
      class="mx-auto max-w-screen-2xl px-8 md:pt-4 pb-8 flex flex-col gap-12"
    >
      <!-- <Map /> -->
      <!-- <Plot /> -->
      <div class="grid grid-cols-3 w-full gap-3 md:gap-6">
        <button @click="onSelectCity(MainCities.Yekaterinburg)">
          <Card
            ...
            :cardClass="
              currentObjectId === MainCities.Yekaterinburg ? 'bg-cyan-600 bg-opacity-30' : ''
            "
          />
        </button>
        <button @click="onSelectCity(MainCities.Tula)">
          <Card
            ...
            img-class="aspect-w-1 aspect-h-1"
            :cardClass="
              currentObjectId === MainCities.Tula ? 'bg-cyan-600 bg-opacity-30' : ''
            "
          />
        </button>
        <button @click="onSelectCity(MainCities.Sevastopol)">
          <Card
            ...
            img-class="aspect-w-1 aspect-h-1"
            :cardClass="
              currentObjectId === MainCities.Sevastopol ? 'bg-cyan-600 bg-opacity-30' : ''
            "
          />
        </button>
      </div>
      <div class="flex gap-6">
        <div class="w-1/2">
          <div class="text-3xl font-semibold">Расширенный список городов</div>
        ...
        <div
          ...
          "
        >
          <div class="text-3xl font-semibold">Список районов</div>
          <div class="text-sm mb-4 font-light text-gray-800">
      ...
      <div v-if="currentObjectId" class="flex flex-col gap-6 pt-10">
        <div class="text-3xl font-semibold">Результаты анализа</div>
            ...
            <div>
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex gap-2">
                  ...
                  <div>
                    Зеленые объекты - положительные (парки, спорт площадки и
                    т.д.)
                  </div>
                </div>
                <div class="flex gap-2">
                  ...
                  <div>
                    Красные объекты - отрицательные (табачные лавки, алкгольные
                    и т.д.)
                  </div>
                </div>
              </div>
        ...
        <div class="text-3xl font-semibold">Расшифровка анализа</div>
        <div v-html="text"></div>
      </div>
    </div>
  </div>
...
const onSelectCity = async (id?: number) => {
  ...
  if (currentObjectId.value === MainCities.Sevastopol) {
